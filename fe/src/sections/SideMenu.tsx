import { useEmailManager } from "../contexts/Contexts";
import { LoadingPoints } from '../components/LoadingPoints';
import { ScrollableP } from '../components/ScrollableP';

const SideMenu = () => {
    const { Mailboxes, isMenuExpanded, selectEmailBox, selectedBox, isLoadingMailBoxes } = useEmailManager();
    return (
        <div className={`${isMenuExpanded ? "flex" : "hidden"} z-2 absolute left-0 sm:flex bg-neutral-950 flex-col items-stretch justify-start box-border h-[calc(100vh-6.25rem)] pt-4 w-50`}>
            {isLoadingMailBoxes ?
                <div className='flex items-center justify-center h-10'><LoadingPoints /></div>
                : Mailboxes.map((box, index) => (
                    <div key={index} className={`h-10 m-0 p-0 flex justify-between bg-neutral-950 hover:drop-shadow-[0.25rem_0_0_dodgerblue] transition-all duration-200 ${selectedBox == index ? "border-b-2 border-blue-500" : ""}`}
                        onClick={() => selectEmailBox(index)}>
                        <ScrollableP textContent={box.Name} color="#ffffff" animationDuration="8"/>
                        {box.NumUnseen > 0 ? <ScrollableP textContent={box.NumUnseen > 0 ? "+999" : (box.NumUnseen + '')} color="#ffffff" animationDuration="8"/> : ""}
                    </div>
                ))
            }


        </div >

    );
};


export default SideMenu;